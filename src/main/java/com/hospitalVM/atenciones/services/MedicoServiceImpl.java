...
public class MedicoServiceImpl implements MedicoService{
    ...
    public Medico findById(Long id) {
        return this.medicoRepository.findById(id).orElseThrow(()-> new MedicoException("Medico no se encontra"));
    }

    @Transactional(readOnly = true)
    @Override
    public Medico findByRun(String run) {
        return this.medicoRepository.findByRun(run).orElseThrow(()-> new MedicoException("Medico no se encontra"));
    }
